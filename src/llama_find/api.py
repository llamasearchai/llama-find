...
def search():
    ...
    try:
        ...
        # Log response
        logger.info(
            f"Search results: {len(ranked_results)} results in {elapsed_time:.2f}s"
        )
        ...
        return (
            jsonify(
                {"error": f"Error processing search request: {str(e)}", "results": []}
            ),
            500,
        )
...
def chat():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"Error processing chat request: {str(e)}", "response": None}
            ),
            500,
        )
...
def create_agent_task(agent_type):
    ...
    try:
        ...
        # Return task ID
        return jsonify(
            {"status": "success", "task_id": task_id, "agent_type": agent_type}
        )
    except Exception as e:
        logger.error(f"Error creating agent task: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
...
def run_server(host: str = None, port: int = None, debug: bool = None):
    ...
    host = host or os.environ.get(
        "LLAMAFIND_HOST", SERVER_CONFIG.get("host", "0.0.0.0")
    )
    port = port or int(
        os.environ.get("LLAMAFIND_PORT", SERVER_CONFIG.get("port", 8080))
    )
