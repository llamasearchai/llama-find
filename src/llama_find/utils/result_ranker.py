...
if USE_MLX:
    try:
        ...
        try:
            ...
            logger.warning(
                "MLX Embeddings package not available, falling back to basic ranking"
            )
    except ImportError:
...
class ResultRanker:
    ...
    def __init__(
        ...
        self.weights = weights or {
            "relevance": self.config.get(
                "relevance_weight", DEFAULT_WEIGHTS["relevance"]
            ),
            "freshness": self.config.get(
                "freshness_weight", DEFAULT_WEIGHTS["freshness"]
            ),
            "authority": self.config.get(
                "authority_weight", DEFAULT_WEIGHTS["authority"]
            ),
            "diversity": self.config.get(
                "diversity_weight", DEFAULT_WEIGHTS["diversity"]
            ),
        }
    ...
    def rank_results(
        ...
        filtered_results = [
            r for r in ranked_results if r.get("_score", 0) >= self.min_score
        ]

        # Log ranking results
        logger.info(
            f"Ranked {len(filtered_results)} results (filtered from {len(ranked_results)})"
        )

        return filtered_results

    def _rank_results_mlx(
        self, results: List[Dict[str, Any]], query: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if text_to_embed.strip():
                    ...
                    relevance_score = self._compute_similarity(
            ...
            ranked_results = sorted(
                scored_results, key=lambda x: x.get("_score", 0), reverse=True
            )
    ...
    def _calculate_diversity_score(
        self, result: Dict[str, Any], index: int, total: int
    ) -> float:
        """
