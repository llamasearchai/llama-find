...
class ElasticsearchManager:
    ...
    def __init__(
        self, host: str = ES_HOST, port: int = ES_PORT, timeout: int = ES_TIMEOUT
    ):
        """
    ...
    def search(
        self, query: str, index_type: str = "web", size: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # Add query
            q = Q(
                "multi_match", query=query, fields=["title^2", "content", "summary^1.5"]
            )
            ...
            logger.info(
                f"Search for '{query}' returned {len(results)} results from {index_name}"
            )
            return results
