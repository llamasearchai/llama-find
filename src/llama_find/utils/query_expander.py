...
if USE_MLX:
    try:
        ...
        try:
            ...
            logger.warning(
                "MLX Embeddings package not available, falling back to basic expansion"
            )
    except ImportError:
...
class QueryExpander:
    ...
    def _expand_query_mlx(self, query: str, max_terms: int) -> str:
        ...
        try:
            ...
            if expansion_terms:
                expanded_query = (
                    query + " " + " ".join([term for term, _ in expansion_terms])
                )
                return expanded_query
    ...
    def _expand_query_basic(self, query: str, max_terms: int) -> str:
        ...
        expansion_terms = set(
            ...
            if term.lower() not in map(str.lower, tokens)
        )
