...
class FunctionRegistry:
    ...
    def register(self, name: Optional[str] = None, description: Optional[str] = None):
        ...
        def decorator(func):
            ...
            func_desc = (
                description or inspect.getdoc(func) or "No description available"
            )
...
class LlamaFunctionCaller:
    ...
    def load_model(self, model_path: str) -> bool:
        ...
        try:
            if self.use_mlx:
                ...
                logger.info(
                    f"Loading LLaMA model without MLX acceleration: {model_path}"
                )
    ...
    async def _process_with_mlx(
        ...
        system_message = (
            system_prompt or "You are a helpful assistant that can call functions."
        )
...
Assistant:"""
    ...
    async def process(self, prompt: str, system_prompt: str = None) -> Dict[str, Any]:
        ...
        if self.use_mlx:
            ...
            return await self._process_with_transformers(
                prompt, functions, system_prompt
            )
        ...
        if "functionCalls" in result and result["functionCalls"]:
            ...
            for func_call in result["functionCalls"]:
                try:
                    ...
                    # Call the function
                    function_output = self.registry.call_function(
                        function_name, args=arguments
                    )

                    # Add function output to result
                    function_results.append(
                        {"name": function_name, "result": function_output}
                    )
                except Exception as e:
